...
void sim_loop( 
    ...
    for (double t = 0; t <= params_sim_ptr->simulation_time; t += params_sim_ptr->dt) {
        pid_target_thrust = compute_thrust(
            ...
            state_aircraft_ptr->altitude, params_sim_ptr->dt
        );
        update_altitude(
            params_sim_ptr, params_aircraft_ptr, 
            state_aircraft_ptr, pid_target_thrust
        );

        // print status
        printf("Thrust (PID): %f  Thrust (real): %f  Alt: %f\n", 
            pid_target_thrust, state_aircraft_ptr->thrust, state_aircraft_ptr->altitude);
    }
}
